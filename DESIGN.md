...
#### Deep Button（濃いめピンクのCTA。ピンク帯の上などPrimaryが沈む場面用）
- Background `#E8B4A8`（Primary Deep） / Text `#3D3B3B` / Border radius `999px`
- Padding desktop `14px 28px` / mobile `13px 22px` / Hover: opacity `0.9`
- 使用は淡ピンク帯上のCTAなど、Primary Buttonでは背景に溶ける場面に限定
...
### Section Label（セクション見出しの小さなpill）
- Background `#F8D2C9`(Primary) / Text `#3D3B3B` / Font size `13px` / weight `600`
- Border radius `8px`（`--radius-label`）/ Padding `5px 14px` / Letter spacing `0.08em`
- **使用箇所は3種のみ**：「LINEでご案内」「利用シーン」「よくある質問」。
  他のコンテンツセクションは見出しのみとする（pillの乱用禁止）

### Logo Mark（花の線画マーク）
- `assets/images/logo-mark.png`（透過PNG・名刺由来の花モチーフ）
- ヘッダー/フッターのワードマーク左に配置。表示高 40px（SP 36px）
- ワードマーク下の欧文は小文字 `hanasatomi`

### Split Story（見出し/写真/本文の3パーツ split）
- PC: 2カラム（テキスト側=見出し+本文+ボタン、メディア側=写真）。`--reverse`で写真左
- **SP: 「見出し → 写真 → 本文 → ボタン」の順に積む**（grid-template-areas）
- SPの見出しは中央寄せ＋下に短い罫（40px・`rgba(184,175,175,0.35)`）

### Story Band（PCで写真が画面端まで届く横長帯。TOP専用）
- Split Storyの拡張。PCではメディア側が**ビューポート端までフルブリード**（角丸なし）、
  テキスト側はコンテナ余白に整列。写真の最小高さ `400px`（PC）
- `--reverse`で写真が左端フルブリード。SPではSplit Storyと同じ縦積み（写真は角丸・左右余白あり）

### TOPページの背景運用（モック準拠）
- TOPは白背景セクションを使わず、**あたたかい生成り `#F1E9E7` を基調**に統一する
- 例外: Hero（ピンクグラデ）／最終CTA（`#F4DAD4` の全幅ピンク帯）
- サブページは従来どおり（白/生成りの交互可）

### Company Table（フラット表）
- 白カードは使わず、セクション背景に直接置くフラットな表（行罫線のみ）
- 金の細罫（`--formal-gold`）を表上部に1本、社名のみ Formal Navy — アクセントはこの2点に限定

### Footer（スリム型）
- PC: 1行水平 … 左=ロゴ（マーク+ワードマーク）／中央=ナビ／右=コピーライト
- SP: 中央寄せ縦 … ロゴ → 縦ナビ → SNSアイコン（LINE/Instagram）→ コピーライト
- SNSアイコンはSPのみ表示。背景 `#F1E9E7`・上罫 `rgba(184,175,175,0.25)` は従来どおり
...
- Border radius `20px` / Border `1px solid rgba(184,175,175,0.35)` / overflow hidden
